...
const questions = [
  {
    q: "How do you get to the end of the line in the repl",
    a: "keyboardcontrol.txt",
  },
  { q: "What is function calling?", a: "functioncalling.txt" },
  {
    q: "How do i output an answer from ollama in well formed JSON",
    a: "functioncalling.txt",
  },
  {
    q: "how can we get a response from a model to be more consistent?",
    a: "functioncalling.txt",
  },
  {
    q: "how do you find the list of shortcuts in the repl",
    a: "keyboardcontrol.txt",
  },
  { q: "how does ollama run", a: "ollamaarchitecture.txt" },
  {
    q: "how many lines in the linux shell script are for CUDA",
    a: "ollamaarchitecture.txt",
  },
  ...
  {
    q: "how do you run the client from a docker exec command",
    a: "ollamaindocker.txt",
  },
  { q: "is a container the same thing as an image", a: "ollamaindocker.txt" },
  {
    q: "tell me about the right way to expense nerf ammunition at datadog",
    a: "ollamaindocker.txt",
  },
  {
    q: "Why is GPU pass-through not possible on Mac with Docker",
    a: "ollamaindocker.txt",
  },
  {
    q: "How do you manage models when running Ollama in Docker",
    a: "ollamaindocker.txt",
  },
  {
    q: "Why might someone prefer to use Docker to run Ollama",
    a: "ollamaindocker.txt",
  },
  {
    q: "What is the basic command to run Ollama in Docker",
    a: "ollamaindocker.txt",
  },
  { q: "On which platforms can Ollama run", a: "ollamaarchitecture.txt" },
  {
    q:
      "How does Ollama differentiate between running as a server and as a client",
    a: "ollamaarchitecture.txt",
  },
  {
    q: "What is the role of the server in Ollama's architecture",
    a: "ollamaarchitecture.txt",
  },
  { q: "Can Ollama run services in the cloud", a: "ollamaarchitecture.txt" },
  {
    q: "Does Ollama use your interactions to improve the model automatically",
    a: "ollamaarchitecture.txt",
  },
  {
    q: "What is the purpose of the save command in Ollama",
    a: "ollamaarchitecture.txt",
  },
];
...
for await (const q of questions) {
  ...
  let success = 0;
  let fail = 0;
  const embedquestion = (await ollama.embeddings({
  ...
  for await (const file of allFiles) {
    ...
    const sortedEmbeddings = embeddings.sort(
      (a, b) =>
        similarity(b.embed, embedquestion) - similarity(a.embed, embedquestion)
    );
  ...
  console.log(
    `Best 5 matches: ${bestMatches
      .sort((a, b) => b.score - a.score)
      .slice(0, 5)
      .map((m) => `${m.file} - ${m.score.toFixed(3)}`)
      .join(", ")}`
  );
  console.log(
    `worst 5 matches: ${bestMatches
